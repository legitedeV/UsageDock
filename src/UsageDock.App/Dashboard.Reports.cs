...
public sealed partial class Dashboard
{
    ...
    {
        var button = Ui.Button("Dodaj połączenie", () => Edit(null), true);
    ...
    private static StackPanel Page(string title, string description)
    {
        ...
        page.Children.Add(new TextBlock { Text = title, FontSize = 26, FontWeight = FontWeights.SemiBold, Foreground = Ui.Text });
        page.Children.Add(new TextBlock { Text = description, FontSize = 14, Foreground = Ui.Muted,
            TextWrapping = TextWrapping.Wrap, Margin = new Thickness(0, 8, 0, 24) });
        return page;
    }
    private static TextBlock SectionTitle(string title) => new()
    {
        Text = title, FontSize = 18, FontWeight = FontWeights.SemiBold, Foreground = Ui.Text,
    ...
    private static string Health(AccountView a) => a.Error != null
        ? a.Snapshot == null ? "Wymaga uwagi" : "Nieaktualne"
        : a.Snapshot == null ? "Oczekiwanie" : "Aktualne";
    ...
    private static string AccountNumber(int count)
    {
        var ending = count % 10;
        var teen = count % 100;
        return $"{count} " + (count == 1 ? "konto" : ending is >= 2 and <= 4 && teen is not (>= 12 and <= 14) ? "konta" : "kont");
    }
    private static UIElement StatusLine(AccountView account)
    {
        var line = Columns(-1, 160);
        var fetched = account.Snapshot?.FetchedAt.ToLocalTime().ToString("dd.MM · HH:mm:ss");
        Cell(line, Truncated(ProviderLabel(account.Profile.Provider) + (fetched == null ? " · Brak odczytu" : " · " + fetched), 13, Ui.Muted), 0);
    ...
    {
        var page = Page("Statystyki", "Ostatni odczyt każdego konta. Limity i koszty pozostają osobne — zakresy organizacji mogą się pokrywać.");
        var all = controller.Accounts;
        var summary = Identify(new TextBlock {
            Text = $"{AccountNumber(all.Count)}     ·     {all.Count(a => a.Snapshot != null && a.Error == null)} aktualnych     ·     {all.Count(a => a.Error != null)} wymaga uwagi     ·     {all.Count(a => a.Snapshot == null && a.Error == null)} oczekujących",
        ...
        foreach (var api in new[] { false, true })
        {
            ...
            page.Children.Add(SectionTitle(api ? "Koszty API" : "Limity abonamentów"));
            foreach (var account in group) page.Children.Add(StatisticsAccount(account, api));
        }
        if (filtered.Length == 0)
            page.Children.Add(Identify(Surface(Ui.Stack(Ui.Label("Brak kont w tym widoku", 18), Ui.Label("Dodaj połączenie lub zmień wyszukiwanie.", 14, Ui.Muted), EmptyAddButton())), "Stats.Empty"));
        return page;
    }
    private UIElement StatisticsAccount(AccountView account, bool api)
    {
        ...
        if (account.Snapshot is not { } snapshot)
            content.Children.Add(Ui.Label(account.Error == null ? "Oczekujemy na pierwszy odczyt." : "Nie udało się odczytać konta. Sprawdź połączenie lub zaloguj się ponownie.", 14, Ui.Muted));
        else if (api)
        {
            var values = Columns(-1, -1, -1);
            Cell(values, Ui.Stack(Ui.Label("Koszt od dostawcy", 13, Ui.Muted), Ui.Label(Money(snapshot.CostUsd), 20)), 0);
            Cell(values, Ui.Stack(Ui.Label("Lokalny budżet / miesiąc", 13, Ui.Muted), Ui.Label(account.Profile.MonthlyBudget.HasValue ? Money(account.Profile.MonthlyBudget) : "Nie ustawiono", 20)), 1);
            Cell(values, Ui.Stack(Ui.Label("Tokeny", 13, Ui.Muted), Ui.Label(Tokens(snapshot.Tokens), 20)), 2);
            ...
            if (snapshot.CostUsd is { } cost && account.Profile.MonthlyBudget is > 0)
            {
                ...
                content.Children.Add(LimitRow("Wykorzystanie budżetu", percent, "Budżet lokalny"));
            }
        }
        else
        {
            ...
            if (snapshot.Windows.Count == 0) content.Children.Add(Ui.Label("Dostawca nie udostępnił limitów.", 14, Ui.Muted));
        }
        if (account.Profile.Provider == ProviderKind.Codex)
        {
            ...
            var manage = Ui.Button("Zarządzaj resetami", () => ShowResetCredits(account.Profile.Id));
        ...
        if (account.Error != null && account.Snapshot != null)
            content.Children.Add(Ui.Label("Ostatnia próba nie powiodła się. Powyżej ostatni dostępny odczyt.", 13, Warning));
    ...
    private UIElement LimitRow(string name, double? percent, string reset, DateTimeOffset? at = null, bool showDate = false)
    {
        ...
        var number = Truncated(percent.HasValue ? $"{percent:0.#}%" : "Niedostępne", 13, percent.HasValue ? Ui.Text : Ui.Muted);
        Cell(row, number, 2);
